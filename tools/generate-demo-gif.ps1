...
for ($frame = 0; $frame -lt $totalFrames; $frame++) {
    ...
    Draw-Text -Graphics $graphics -Text 'Supported directional cursors follow the Primary mouse button setting automatically.' -Font $bodyFont -Brush $textMuted -X 92 -Y 108
    ...
    Draw-Text -Graphics $graphics -Text 'When set to Right, the supported directional cursors are mirrored. When set back to Left, the default cursors return.' -Font $smallFont -Brush $textMuted -X 132 -Y 256
    ...
    Draw-Text -Graphics $graphics -Text 'Directional cursor state' -Font $bodyFont -Brush $textPrimary -X 640 -Y 378
